...
def calculate_metrics(pred, target):
    ...
    target = torch.where(target == 0, -1, target)  # 将真实标签中的背景像素点替换成负值
    true_positive = ((pred >= 0.5) & (target == 1)).sum().item()
    true_negative = ((pred < 0.5) & (target == 0)).sum().item()
    false_positive = ((pred >= 0.5) & (target == -1)).sum().item()   # 只考虑真实标签中的阳性像素点
...
# 在屏幕和文件中输出
sys.stdout = open('/home/ug2020/ug520111910171/bioalgorithm/Brain/U-Net/test_only_mask.txt', 'w')
