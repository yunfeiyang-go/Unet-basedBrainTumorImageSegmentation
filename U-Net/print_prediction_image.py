...
def predict_image(image_path, mask_path):
    ...
    pred[pred >= 0.5] = 255  # 将概率大于等于 0.5 的像素设为前景
    pred[pred < 0.5] = 0    # 将概率小于 0.5 的像素设为背景

    # 将 numpy 数组转换为 PIL.Image 对象，并保存到指定路径
    pred_image = Image.fromarray(pred[0][0]).convert('L')  # 转换为 8 位灰度图像
